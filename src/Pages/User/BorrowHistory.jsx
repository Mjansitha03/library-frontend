...
const BorrowHistory = () => {
  ...
  useEffect(() => {
    getBorrowHistory().then((res) => setHistory(res.data));
  }, []);

  if (!history.length)
    return <p className="text-center mt-10 text-gray-500">No history found</p>;

  return (
    <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
      {history.map((b) => (
        <div key={b._id} className="bg-white rounded-xl shadow">
          <img src={b.book.image} className="h-48 w-full object-cover" />
